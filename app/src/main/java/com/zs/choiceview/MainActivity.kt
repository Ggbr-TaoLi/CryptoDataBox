...
//        datas = doJsonReader() //获取数据
        //从网络上获取数据
        val http = "https://api2.binance.com/api/v3/ticker/24hr"
        datas = doGet(http)
        initAllData(datas)
        initDatas = datas
    }
...
//        mData.add(GridItemBean("3" , "10000+"))
        ...
        var layoutManager: androidx.recyclerview.widget.GridLayoutManager =
            recycler_grid?.layoutManager as androidx.recyclerview.widget.GridLayoutManager
...
//        var data3 = mAdapter?.getMultiChoiceItem(3)
    ...
    fun doJsonReader(): MutableList<BaseBean> {
        ...
        try {
            ...
            val isr =
                InputStreamReader(this.javaClass.classLoader.getResourceAsStream(url), "UTF-8")
            //从assets获取json文件
            ...
            for (i in 0 until array.length()) {
                ...
                d.add(
                    BaseBean(
                        stud.getString("symbol"),
                        stud.getString("priceChange"),
                        stud.getString("count"),
                        temp,
                        "http://img4.imgtn.bdimg.com/it/u=1758683838,870080854&fm=26&gp=0.jpg"
                    )
                )
                bfr.close()
                isr.close()
    ...
    fun doGet(urlStr: String?): MutableList<BaseBean> {
        var d: MutableList<BaseBean> = ArrayList<BaseBean>()
        val sb = StringBuffer()
        try {
            val url = URL(urlStr)
            ...
            if (conn.responseCode == 200) {
                ...
                while (isr.read(buf).also { len = it } != -1) {
